...
import {subscribeToUpdates} from "../../Store/action";
...
const Subscribe = () => {
    ...
    const handleSubscribe = () => {
        (dispatch as any)(subscribeToUpdates())
        setEmail("")
    }
    return (
        <div className="mt-5 flex items-center bg-mine-shaft-900 mx-20 sm-mx:mx-5 py-3 rounded-xl justify-around flex-wrap">
            ...
            <div className="flex rounded-xl gap-4 bg-mine-shaft-700 px-3 py-2 items-center xs-mx:flex-col ">
                <TextInput value={email} onChange={(e)=>setEmail(e.currentTarget.value)} size="xl" className="[_&input]:text-mine-shaft-200 font-semibold" variant="unstyled" placeholder="Your@gmail.com" />
                <Button onClick={handleSubscribe} className="!rounded-lg" variant="filled" size={matches1?"":matches?"md":"lg"} color="brightSun.4" >Subscribe</Button>
            </div>
        </div>
    )
}
